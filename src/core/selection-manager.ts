export class SelectionManager {
  ...
  hasFormatting(tagNames: string[]): boolean {
    ...
    if (range.collapsed) {
      return this.hasFormattingAtCursor(tagNames);
    }
    
    // For non-collapsed selections, check all possible locations where formatting might be
    return (
      this.hasFormattingInAncestors(range, tagNames) || 
      this.hasFormattingInContents(range, tagNames)
    );
  }
  ...
  private hasFormattingInContents(range: Range, tagNames: string[]): boolean {
    // Clone the range contents to inspect them
